...
class CTrainTestSet:
    ...
    def Split(self, 
              test_size=0.2, 
            #   test_size=0.15,
              valid_set=None, test_set=None, random_state=12345):
        """
        ...
        if test_set != 'no':
            if test_size < 1:
                if valid_set == 'no':
                    ...
                    # train1X, self.testX, train1Y, self.testY, train1Filenames, self.testFilenames = \
                    #     train_test_split(self.X, self.y, self.filenames, test_size=test_ratio,
                    #                      random_state=random_state,
                    #                      shuffle=True, stratify=self.y)
                    # self.trainX, self.valX, self.trainY, self.valY, self.trainFilenames, self.valFilenames = \
                    #     train_test_split(train1X, train1Y, train1Filenames,
                    #                      test_size=validation_ratio / (train_ratio + validation_ratio),
                    #                      random_state=random_state, shuffle=True, stratify=train1Y)
